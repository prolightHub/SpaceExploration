...
gameObjects.used = {};
// Might be faster just to feed in the cell (list) instead of calling the func all the time!
gameObjects.loopThrough = function(left, right, up, down, func)
{
    ...
    for(col = left; col <= right; col++)
    {
        for(row = up; row <= down; row++)
        {
            ...
            {
                func(
                    this[this.references[cell[i].arrayName]][cell[i].id], 
                    cell[i].id, 
                    this[this.references[cell[i].arrayName]], 
                    cell[i]);
            }
        }
    }
};
gameObjects.cutLoopThrough = function(left, right, up, down, func)
{
    // Keep numbers inside cameraGrid
    left = Math.max(left, 0);
    right = Math.min(right, cameraGrid.maxCol);
